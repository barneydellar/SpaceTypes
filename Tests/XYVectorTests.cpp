...
TEST_CASE("XYVectors can be constructed") { View::XYVector v; }
...
TEST_CASE(
    "XYVectors throw with the right message if random access is too high") {
  const Image::XYVector v;
...
TEST_CASE(
    "XYVector and NormalizedVector can be compared using equal: different") {
  const View::XYVector v(1, 1);
  const View::NormalizedVector v2(1, 0, 0);
  CHECK(!(v == v2));
}
TEST_CASE(
    "XYVector and NormalizedXYVector can be compared using equal: different") {
  const View::XYVector v(1, 1);
  const View::NormalizedXYVector v2(1, 0);
  CHECK(!(v == v2));
}
...
TEST_CASE(
    "XYVector and NormalizedVector can be compared using inequality: same") {
  const View::XYVector v(1, 0);
  const View::NormalizedVector v2(1, 0, 0);
  CHECK(!(v != v2));
}
TEST_CASE(
    "XYVector and NormalizedXYVector can be compared using inequality: same") {
  const View::XYVector v(1, 0);
  const View::NormalizedXYVector v2(1, 0);
  CHECK(!(v != v2));
}
...
#ifndef IGNORE_SPACE_STATIC_ASSERT
TEST_CASE(
    "XYVectors from different spaces cannot be compared using inequality") {
  const View::XYVector v;
  const Image::Vector v_v;
  const Image::NormalizedVector v_n;
  const Image::NormalizedXYVector v_n_xy;
...
TEST_CASE(
    "XYVectors can have other XYVectors subtracted to produce a XYVector") {
  View::XYVector v;
...
TEST_CASE(
    "XYVectors can have other XYvectors added to produce another XYVector") {
  View::XYVector v;
  const View::NormalizedXYVector v_n_xy;
  const View::XYVector v_xy;
...
TEST_CASE(
    "XYVectors can have other 3D vectors added to produce another Vector") {
  View::XYVector v;
  const View::Vector v_v;
  const View::NormalizedVector v_n;
...
TEST_CASE(
    "XYVectors can have other vectors crossed to produce another Vector") {
  View::XYVector v;
  const View::Vector v_v;
  const View::NormalizedVector v_n;
  const View::NormalizedXYVector v_n_xy;
