...
TEST_CASE("Points can be constructed") { View::Point p; }
...
#ifndef IGNORE_SPACE_STATIC_ASSERT
TEST_CASE(
    "Points and vectors in different spaces cannot be subtracted in place") {
  View::Point p;
  const Image::Vector v_v;
  const Image::NormalizedVector v_n;
  const Image::NormalizedXYVector v_n_xy;
...
TEST_CASE("Points from Spaces that do not support XY cannot have their z-value "
          "removed") {
...
TEST_CASE(
    "Points can be converted from one space to another ignoring translation") {
...
TEST_CASE(
    "Points can be converted from one space to another to produce a Point") {
  const TransformManager tm;
