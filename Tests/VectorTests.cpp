...
TEST_CASE("Vectors can be constructed") { View::Vector v; }
...
TEST_CASE(
    "Vector and NormalizedVector can be compared using equal: different") {
  const View::Vector v(1, 0, 1);
  const View::NormalizedVector v2(1, 0, 0);
  CHECK(!(v == v2));
}
TEST_CASE(
    "Vector and NormalizedXYVector can be compared using equal: different") {
  const View::Vector v(1, 0, 1);
  const View::NormalizedXYVector v2(1, 0);
  CHECK(!(v == v2));
}
...
TEST_CASE(
    "Vector and NormalizedVector can be compared using inequality: same") {
  const View::Vector v(1, 0, 0);
  const View::NormalizedVector v2(1, 0, 0);
  CHECK(!(v != v2));
}
TEST_CASE(
    "Vector and NormalizedXYVector can be compared using inequality: same") {
  const View::Vector v(1, 0, 0);
  const View::NormalizedXYVector v2(1, 0);
  CHECK(!(v != v2));
}
...
TEST_CASE(
    "Vector and NormalizedVector can be compared using inequality: different") {
  const View::Vector v(1, 0, 1);
  const View::NormalizedVector v2(1, 0, 0);
  CHECK(v != v2);
}
...
TEST_CASE(
    "Vectors can have other vectors subtracted to produce another Vector") {
  View::Vector v;
...
TEST_CASE(
    "Vectors can be converted from one space to another ignoring translation") {
...
TEST_CASE(
    "Vectors can be converted from one space to another to produce a Vector") {
  const TransformManager tm;
