...
TEST_CASE("XYPoints can be constructed") { View::XYPoint p; }
...
TEST_CASE(
    "XYPoints throw with the right message if random access is too high") {
  const Image::XYPoint p;
...
#ifndef IGNORE_SPACE_STATIC_ASSERT
TEST_CASE(
    "XYPoints from different spaces cannot be compared using inequality") {
  const View::XYPoint p;
  const Image::Point p_p;
  const Image::XYPoint p_xy;
...
TEST_CASE(
    "XYPoints and vectors in different spaces cannot be subtracted in place") {
  View::XYPoint p;
  const Image::Vector v_v;
  const Image::NormalizedVector v_n;
  const Image::NormalizedXYVector v_n_xy;
...
TEST_CASE(
    "XYPoints can have XY vectors subtracted to produce another XYPoint") {
  View::XYPoint p;
  const View::NormalizedXYVector v_n_xy;
  const View::XYVector v_xy;
...
TEST_CASE(
    "XYPoints can have other XYPoints subtracted to produce an XYVector") {
  View::XYPoint p;
  const View::XYPoint p_xy;
...
TEST_CASE(
    "XYPoints can be converted from one space to another to produce a Point") {
  const TransformManager tm;
