...
TestVector ReturnFirstImpl(std::vector<TestVector> v) { return v.front(); }
...
TEST_CASE("Collections of XYPoints can be created using initalizer lists") {
  const std::vector<View::XYPoint> points{{1, 1}, {2, 2}};
...
TEST_CASE("Collections of Vectors can be created using initalizer lists") {
  const std::vector<View::Vector> vectors{{1, 1, 1}, {2, 2, 2}};
...
TEST_CASE(
    "Collections of NormalizedVectors can be created using initalizer lists") {
  const std::vector<View::NormalizedVector> vectors{{1, 0, 0}, {0, 1, 0}};
...
TEST_CASE("Collections of XYVectors can be created using initalizer lists") {
  const std::vector<View::XYVector> vectors{{1, 1}, {2, 2}};
...
TEST_CASE("Strongly typed collections can be used in templated functions: Data "
          "Space") {
  const std::vector<Data::Point> points{Data::Point{1, 1, 1},
                                        Data::Point{2, 2, 2}};
...
TEST_CASE("Strongly typed collections can be transformed for use in "
  ...
  std::transform(points.cbegin(), points.cend(), std::back_inserter(impls),
                 [](auto v) { return static_cast<TestVector>(v); });

  const auto first = ReturnFirstImpl(impls);

  CHECK(first.m_values[0] ==
        static_cast<TestVector>(points.front()).m_values[0]);
  CHECK(first.m_values[1] ==
        static_cast<TestVector>(points.front()).m_values[1]);
  CHECK(first.m_values[2] ==
        static_cast<TestVector>(points.front()).m_values[2]);
}
