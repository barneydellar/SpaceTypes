...
template <typename Space> typename Space::Point ReturnFirst(std::vector<typename Space::Point> v) { return v.front(); }
...
TEST_CASE("Strongly typed collections can be transformed for use in non-templated functions") {
    ...
    std::transform(points.cbegin(), points.cend(), std::back_inserter(impls), [](auto v) { return static_cast<TestVector>(v); });
