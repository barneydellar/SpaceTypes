...
public:
  ...
  explicit Point(const UnderlyingData &v) noexcept {
    std::copy(implementation::cbegin(v), implementation::cend(v), begin());
  }
  ...
  [[nodiscard]] double *end() noexcept {
    return reinterpret_cast<double *>(&underlyingData) + 3;
  }
  ...
  [[nodiscard]] const double *cend() const noexcept {
    return reinterpret_cast<const double *>(&underlyingData) + 3;
  }
  ...
  template <int I>
    requires ValidFor3dAt<I>
  [[nodiscard]] double at() const {
    return operator[](I);
  }

  [[nodiscard]] bool
  operator==(const Point<ThisSpace, UnderlyingData> &other) const noexcept {
    return std::equal(cbegin(), cend(),
                      implementation::cbegin(other.underlyingData), Equality);
  }
  [[nodiscard]] bool
  operator==(const XYPoint<ThisSpace, UnderlyingData> &other) const noexcept {
    return std::equal(cbegin(), cend(),
                      implementation::cbegin(other.underlyingData), Equality);
  }
  ...
  auto
  operator-=(const NormalizedVector<ThisSpace, UnderlyingData> &rhs) noexcept {
    Sub(underlyingData, rhs.underlyingData);
    return *this;
  }
  auto operator-=(
  ...
  [[nodiscard]] auto
  operator-(const Point<ThisSpace, UnderlyingData> &rhs) const noexcept {
    Vector<ThisSpace, UnderlyingData> v(X(), Y(), Z());
    Sub(v.underlyingData, rhs.underlyingData);
    return v;
  }
  [[nodiscard]] auto
  operator-(const XYPoint<ThisSpace, UnderlyingData> &rhs) const noexcept {
    Vector<ThisSpace, UnderlyingData> v(X(), Y(), Z());
    Sub(v.underlyingData, rhs.underlyingData);
    return v;
  }
  ...
  [[nodiscard]] auto
  ConvertTo(const TransformManager &transform_manager) const {
    return Point<OtherSpace, UnderlyingData>(
        transform_manager.template TransformPoint<ThisSpace, OtherSpace>(
            static_cast<UnderlyingData>(*this)));
  }

  [[nodiscard]] auto ToXY() const
    requires ThisSpace::supportsXY
  {
    return XYPoint<ThisSpace, UnderlyingData>(X(), Y());
  }

  friend std::ostream &
  operator<<(std::ostream &os, const Point<ThisSpace, UnderlyingData> &item) {
    const auto space = SpaceTypeNameMap<ThisSpace>::name;
    os << space << "::Point (" << item.X() << ", " << item.Y() << ", "
       << item.Z() << ")";
    return os;
  }
  ...
  operator!=(const Point<OtherSpace, UnderlyingData> &) const noexcept {
    return StaticAssert::invalid_space{};
  }
  template <typename OtherSpace>
    requires DifferentSpaces<OtherSpace, ThisSpace>
  StaticAssert::invalid_space
  operator!=(const XYPoint<OtherSpace, UnderlyingData> &) const noexcept {
    return StaticAssert::invalid_space{};
  }
  ...
  StaticAssert::invalid_point_vector_equality operator==(
  ...
  StaticAssert::invalid_point_to_point_addition
  ...
  StaticAssert::invalid_point_from_point_subtraction
  operator-=(const Point<ThisSpace, UnderlyingData> &) const noexcept {
  ...
  [[nodiscard]] StaticAssert::XYVector_not_supported ToXY() const noexcept
    requires ThisSpace::doesNotSupportXY
  {
