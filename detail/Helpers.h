...
static void Add(UnderlyingData &self, const UnderlyingData &other) {
  std::transform(cbegin(self), cend(self), cbegin(other), begin(self),
                 std::plus<>());
}
...
static void Scale(UnderlyingData &self, const double &d) {
  std::transform(cbegin(self), cend(self), begin(self),
                 [d](auto v) { return v * d; });
}
...
static double Dot(const UnderlyingData &A, const UnderlyingData &B) noexcept {
  return std::transform_reduce(
      cbegin(A), cend(A), cbegin(B), 0.0,
      [](auto accumulation, auto v) { return accumulation + v; },
      [](auto v1, auto v2) { return v1 * v2; });
}

template <typename UnderlyingData>
static auto Cross_internal(const UnderlyingData &A,
                           const UnderlyingData &B) noexcept {
...
[[nodiscard]] static double Mag_internal(const UnderlyingData &i) noexcept {
  ...
  return std::sqrt(std::accumulate(cbegin(i), cend(i), 0.0,
