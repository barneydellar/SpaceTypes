...
namespace Space::detail {
    ...
    public:
        ...
        constexpr explicit NormalizedVector(const std::array<double, 3> value) : _base(value) { Normalise();}
        constexpr explicit NormalizedVector(const double x, const double y, const double z) : _base(x, y, z) { Normalise(); }
        constexpr explicit NormalizedVector(const double x, const double y) : _base(x, y, 0) { Normalise(); }

        constexpr operator Vector<Space>() const {
            return Space::Vector(_base::m_values);
        }

        //-------------------------------------------------------------------------------------

        constexpr bool operator == (const NormalizedVector<Space>& other) const {
            return std::equal(_base::m_values.cbegin(), _base::m_values.cend(), other.m_values.cbegin());
        }
        constexpr bool operator == (const Vector<Space>& other) const {
            return std::equal(_base::m_values.cbegin(), _base::m_values.cend(), other.m_values.cbegin());
        }
        ...
        constexpr typename Space::Vector operator+(const NormalizedVector<Space>& rhs) const {
            return Space::Vector(_base::SumArrays(_base::m_values, rhs.m_values));
        }
        constexpr typename Space::Vector operator+(const Vector<Space>& rhs) const {
            return Space::Vector(_base::SumArrays(_base::m_values, rhs.m_values));
        }
        template <typename WrongSpace>
        ...
        constexpr StaticAssert::invalid_normalized_vector_scale operator*=(const double&) const {
            return StaticAssert::invalid_normalized_vector_scale{};
        }

        template <typename OtherSpace>
        constexpr StaticAssert::invalid_normalized_vector_addition operator+=(const Vector<OtherSpace>&) const {
            return StaticAssert::invalid_normalized_vector_addition{};
        }

        template <typename OtherSpace>
        constexpr StaticAssert::invalid_normalized_vector_addition operator+=(const NormalizedVector<OtherSpace>&) const {
            return StaticAssert::invalid_normalized_vector_addition{};
        }
        ...
        constexpr double Dot(const NormalizedVector<Space>& other) const {
            return _base::DotArrays(_base::m_values, other.m_values);
        }
        constexpr double Dot(const Vector<Space>& other) const {
            return _base::DotArrays(_base::m_values, other.m_values);
        }
        template <typename WrongSpace>
        ...
        // Crossing
        constexpr typename Space::NormalizedVector operator*(const NormalizedVector<Space>& rhs) const {
        ...
        constexpr typename Space::NormalizedVector Cross(const NormalizedVector<Space>& other) const {
            return Space::NormalizedVector(_base::CrossArrays(_base::m_values, other.m_values));
        }

        constexpr typename Space::Vector Cross(const Vector<Space>& other) const {
            return Space::Vector(_base::CrossArrays(_base::m_values, other.m_values));
        }

        //-------------------------------------------------------------------------------------

        constexpr StaticAssert::normalized_vectors_do_not_support_non_const_iteration begin() const {
            return StaticAssert::normalized_vectors_do_not_support_non_const_iteration{};
        }

        constexpr StaticAssert::normalized_vectors_do_not_support_non_const_iteration end() const {
            return StaticAssert::normalized_vectors_do_not_support_non_const_iteration{};
        }
