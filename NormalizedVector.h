...
public:
  ...
  explicit NormalizedVector(const UnderlyingData &v) noexcept(false) {
    std::copy(implementation::cbegin(v), implementation::cend(v),
              begin(underlyingData));
    Normalize();
  }
  NormalizedVector(const double x, const double y,
  ...
  operator<<(std::ostream &os,
    ...
    os << space << "::NormalizedVector (" << item.X() << ", " << item.Y()
       << ", " << item.Z() << ")";
    return os;
  }
#ifndef IGNORE_SPACE_STATIC_ASSERT
  ...
  StaticAssert::invalid_normalized_vector_addition operator+=(
  ...
  StaticAssert::invalid_normalized_vector_subtraction operator-=(
      const NormalizedVector<ThisSpace, UnderlyingData> &) const noexcept {
    return StaticAssert::invalid_normalized_vector_subtraction{};
  }
  StaticAssert::invalid_normalized_vector_subtraction operator-=(
...
private:
  void Normalize() {
    ...
    std::transform(cbegin(), cend(), begin(underlyingData),
